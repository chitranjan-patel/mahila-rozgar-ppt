...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', 'Noto Sans Devanagari', sans-serif; }
        ...
        .slider-container {
            width: 95vw; height: 90vh; position: relative; overflow: hidden; 
        ...
        .text-section {
            flex: 1; padding: 4rem 5rem; display: flex; flex-direction: column; justify-content: center; z-index: 2;
            background: var(--white);
        }

        /* PREMIUM IMAGE SECTION WITH AMBIENT BLUR */
        .image-section {
            flex: 1.2;
            position: relative;
            overflow: hidden;
        ...
        .img-front {
            ...
            background-position: center;
            z-index: 1;
            transform: scale(0.92); /* Makes it float beautifully */
            filter: drop-shadow(0 25px 40px rgba(0,0,0,0.5));
            border-radius: 12px;
            overflow: hidden;
        }

        h1 { color: var(--primary); font-size: 3.2rem; margin-bottom: 1.5rem; line-height: 1.3; font-weight: 700; }
        ...
        ul li { position: relative; padding-left: 1.5rem; margin-bottom: 0.8rem; font-size: 1.05rem; color: var(--dark); }
        ul li::before { content: '✔'; color: var(--secondary); position: absolute; left: 0; font-weight: bold; }

        .tagline {
            display: inline-block; background: var(--primary); color: var(--white);
            padding: 1rem 2rem; border-radius: 50px; font-size: 1.1rem; font-weight: 600;
            box-shadow: 0 10px 20px rgba(0,75,147,0.2);
        }

        .demo-box {
            background: var(--light); border-left: 6px solid var(--secondary);
            padding: 1.5rem; border-radius: 0 12px 12px 0; font-size: 1.05rem; color: var(--primary); font-weight: 600;
        }
...
def build_slide(i, content_html):
    active = ' active' if i == 1 else ''
    url = f"images/slide{i}.png"
    return f"""
        <!-- Slide {i} -->
        <div class="slide{active}">
            <div class="slide-content-wrapper">
                <div class="image-section">
                    <div class="img-blur" style="background-image: url('{url}');"></div>
                    <div class="img-front" style="background-image: url('{url}');"></div>
                </div>
...
slides_content = [
    ...
    """                    <h2>कमाई के संभावित स्रोत</h2>
                    ...
                    <p style="font-size: 1.1rem; margin-top: 1rem;">यदि एक महिला 3-4 छोटे व्यवसायों का डिजिटल मार्केटिंग सँभालती है, तो वह आसानी से घर बैठे <b>₹10,000 से ₹15,000</b> प्रति माह कमा सकती है।</p>""",
...
for i in range(15):
    html_content += build_slide(i+1, slides_content[i])
...
with open(r'C:\Users\Chitranjan Kumar\Desktop\ppt\web\index.html', 'w', encoding='utf-8') as f:
    f.write(html_content)

print('Updated with ambient blur floating layout')
